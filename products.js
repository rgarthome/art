...
async function productsApiHandler(req, res) {
  try {
    ...
    if (mode === "categories") {
      return sendJson(res, 200, {
        categories: cache.categories || ["Toate"],
    ...
    if (search) {
      filtered = filtered.filter(p => p.searchText.includes(search));
    }
...
module.exports = { productsApiHandler };
